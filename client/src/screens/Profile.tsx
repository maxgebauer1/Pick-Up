import React, { useState } from 'react';
import { Bell, ChevronRight, LogOut } from 'lucide-react';
import { useStore, myParticipant } from '../data/store';
import { useAuth } from '../context/AuthContext';
import { Avatar } from '../components/Avatar';
import { GameCard } from '../components/GameCard';
import { enablePush, pushSupported } from '../lib/push';
import { Player } from '../types';

export const Profile: React.FC = () => {
  const { games } = useStore();
  const { user, logout } = useAuth();
  const [pushState, setPushState] = useState<'off' | 'busy' | 'on' | 'denied' | 'unsupported'>(
    pushSupported() ? 'off' : 'unsupported'
  );

  if (!user) return null;

  const me: Player = { id: user.id, name: user.name, initials: initials(user.name), color: '#186640' };

  const myGames = games
    .filter((g) => myParticipant(g, user.id))
    .sort((a, b) => new Date(a.startsAt).getTime() - new Date(b.startsAt).getTime());

  const togglePush = async () => {
    if (pushState === 'on' || pushState === 'busy') return;
    setPushState('busy');
    const result = await enablePush();
    setPushState(result === 'subscribed' ? 'on' : result === 'denied' ? 'denied' : result === 'unsupported' ? 'unsupported' : 'off');
  };

  return (
    <div className="mx-auto max-w-md px-4 pt-8 pb-28">
      <div className="flex flex-col items-center text-center">
        <Avatar player={me} size={76} />
        <h1 className="text-[22px] font-extrabold mt-3">{user.name}</h1>
        <p className="text-[13px] text-muted mt-0.5">{user.city || 'Venice, CA'}</p>
      </div>

      {/* notifications */}
      <div className="card mt-7 p-4">
        <div className="flex items-center gap-3">
          <span className="w-9 h-9 rounded-pill bg-green-soft flex items-center justify-center">
            <Bell size={17} className="text-green" />
          </span>
          <div className="flex-1">
            <p className="text-[14px] font-extrabold">Game reminders</p>
            <p className="text-[12.5px] text-muted">
              {pushState === 'denied'
                ? 'Blocked — enable notifications in your browser settings.'
                : pushState === 'unsupported'
                ? 'Not supported on this device.'
                : 'Get a nudge to confirm before each game.'}
            </p>
          </div>
          <button
            onClick={togglePush}
            disabled={pushState === 'unsupported' || pushState === 'busy'}
            className={`w-12 h-7 rounded-pill p-0.5 transition-colors disabled:opacity-50 ${pushState === 'on' ? 'bg-green' : 'bg-line'}`}
            aria-label="Toggle reminders"
          >
            <span
              className="block w-6 h-6 rounded-pill bg-white transition-transform"
              style={{ transform: pushState === 'on' ? 'translateX(20px)' : 'translateX(0)' }}
            />
          </button>
        </div>
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-line text-[13px]">
          <span className="text-muted font-semibold">Text me reminders too</span>
          <span className="flex items-center gap-1 text-faint font-bold">Coming soon <ChevronRight size={14} /></span>
        </div>
      </div>

      {/* my games */}
      <h2 className="text-[13px] font-extrabold mt-7 mb-3">Your games</h2>
      {myGames.length === 0 ? (
        <p className="text-[14px] text-muted text-center py-10">You haven't joined a game yet.</p>
      ) : (
        <div className="flex flex-col gap-3">
          {myGames.map((g) => (
            <GameCard key={g.id} game={g} />
          ))}
        </div>
      )}

      <button
        onClick={logout}
        className="mt-8 w-full flex items-center justify-center gap-2 text-[14px] font-bold text-muted py-3"
      >
        <LogOut size={16} /> Sign out
      </button>
    </div>
  );
};

function initials(name: string) {
  const parts = name.trim().split(/\s+/);
  return (parts.length >= 2 ? parts[0][0] + parts[1][0] : name.slice(0, 2)).toUpperCase();
}
